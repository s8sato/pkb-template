...
→ 関連: R-004, R-006, R-012, R-013, F-003, N-001, N-002, N-007 / vault-design §3, §11
...
| Logseq を母艦にする | 却下 | **ファイル形式の可搬性**: 旧 Markdown 版は可搬性を保てるが、公式の移行先である DB 版では `db.sqlite` が正本となり、プレーン Markdown の Git 差分管理・Quartz 直接公開の前提が崩れる。**パフォーマンス**: 旧 Markdown 版は大規模グラフで重くなりやすく、2,000 ファイル台で UI クラッシュ・30,000 ファイル超で起動不能の報告がある。DB 版はパフォーマンス改善方向だが上記の可搬性問題を抱え、リリース状況も不安定（2025年12月の beta が Latest、nightly は pre-release 扱い）。 | N-001, N-008, R-012, R-013, R-018 |
...
→ 関連: R-014, R-015, R-016, R-017, F-005, F-006, F-009, F-010, N-006 / §5, §6
...
**採用: 著作権制約のある `## Kindle Highlights` セクションは、ノートを公開しても抽出時に機械的に除去する。**
→ 関連: R-009, R-019, R-020, F-010 / §5, §8, §13

| 対象 | 判断 | 根拠 | 関連 |
| --- | --- | --- | --- |
| `reading/` ノートを丸ごと公開禁止にする | 却下 | ノート単位の禁止は、自分の考察（`## Notes` / `## Links`）まで巻き添えで非公開にし、選択公開（R-019）の粒度を粗くする。除去はハイライト本文に限定すれば足りる。 | R-019, R-020 |
| `publish: false` を frontmatter で必須化して保護する | 却下 | 人手の frontmatter は誤って `true` に変えられ得る。著作権保護は人手に依存せず、抽出ツール（`sync-public.mjs`）と CI が機械的に強制する。 | R-014, R-020 |
| セクション除去をユーザー判断（規約）に委ねる | 却下 | 見落とし時に著作権侵害が公開される。ツールが強制除去し、CI が除去漏れを検査する多層防御とする。 | R-020 |

## 5. ディレクトリ構成

**採用: `ideas/` と `reading/` のフラット構成とし、階層よりリンクで接続する。**
→ 関連: R-002, R-003, R-006, R-007, R-008, R-009, R-010, R-011, F-001, F-002, F-004, N-003 / §3, §4
...
→ 関連: R-001, R-002, スコープ（Web クリップ） / §4, §11, §12
...
→ 関連: R-001, R-013, N-004, N-005 / §11, §12, §13
